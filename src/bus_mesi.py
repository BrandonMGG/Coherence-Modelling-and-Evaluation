...
class Bus:
    ...
    def _process_readmiss(self, core_id, local_cache, block_id, state, address, data):
        # If I do read and if I am writing on M or E I must do WB
        if state in ["M", "E"]:
        ...
        # If sponsors state is E or M then set block state to S
    ...
    def _seek_owned(self, local_address, requester_id):
        ...
        for core_id in range(NUM_CPU):
            if core_id != requester_id:
                ...
                # If block is M or E, then set state to E and give value
                if local_address == address and state in ["M", "E"]:
                    block.set_state("E")
                    time.sleep(CACHE_RD_DELAY)
